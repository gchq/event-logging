...
public class XMLWriter implements ContentHandler {
    ...
    private static final boolean[] specialInText; // lookup table for special
                                                    // characters in text
    private static final boolean[] specialInAtt; // lookup table for special
    ...
    static {
        ...
        for (int i = 0; i <= 31; i++)
            specialInText[i] = true; // allowed in XML 1.1 as character
                                        // references
        ...
        for (int i = 0; i <= 31; i++)
            specialInAtt[i] = true; // allowed in XML 1.1 as character
                                    // references
        for (int i = 32; i <= 127; i++)
    ...
    private int line = 0; // line and column measure the number of lines and
                            // columns
    ...
    public void startDocument() throws SAXException {
        try {
            if (outputXMLDecl) {
                writer.write("<?xml version=\"" + version.getOutput()
                        + "\" encoding=\"UTF-8\"?>");
                afterEndTag = true;
            }
    ...
    private void writeCharacters(final boolean neverNormalize)
            throws SAXException {
        try {
    ...
    private void writeAtts(final Writer writer, final Attributes atts)
            throws SAXException {
        if (sortAtts) {
    ...
    private void writeAttribute(final Writer writer, final String name,
    ...
    private void writeEscape(final Writer writer, final String chars,
            final boolean inAttribute) throws java.io.IOException, SAXException {
        int segstart = 0;
        final boolean[] specialChars = (inAttribute ? specialInAtt
                : specialInText);
    ...
    private void outputCharacterReference(final Writer writer, final int charval)
            throws IOException {
        writer.write("&#x");
